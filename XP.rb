...
begin
  ...
  ROOT_DIR = "/home/naomi/xplab/"
  DATA_DIR = ROOT_DIR + "/data/"
end
load ROOT_DIR + "util.rb"
load ROOT_DIR + 'lib/rpc/xp.rb'
...
module XP
  ...
  class HTTPRPCRepl
    @cli = nil
    def initialize
      @cli = XP::RPC::Client.new("user" => ::XP::Secret::HTTP_RPC_USER, "pass" => ::XP::Secret::HTTP_RPC_PASS, "host" => ::XP::Secret::HTTP_RPC_HOST, "port" => ::XP::Secret::HTTP_RPC_PORT)
    end
  ...
  class RPCRepl < HTTPRPCRepl
    def initialize
      ...
      begin
        ...
        puts "chainstats initialization FAILURE!!: #{e}"   
      end
      self
    end
    ...
    def _api(mode,*args)      
      begin
        ...
        url_string = "#{mode == :main ? ::XP::INSIGHT_URL : ::XP::INSIGHT_URL_TEST}api/#{prms.join('/')}?#{hprm.join('&')}"
    ...
    def tx(txid)
      ...
      dattr = {}
        begin
      if txid == ::XP::GENESIS_COINBASE_TXID
        dattr = {"blockhash" => ::XP::GENESIS_BLOCK_HASH}
        rawtx = getblock(::XP::GENESIS_BLOCK_HASH,false)[162..-1]
      else
  ...
  class Block < CoinPrim
    ...
    def inspect
      "#<XP::Block full=#{is_full?} height=#{@attrs['height']} hash=#{@attrs['hash']}>"
    end    
  end
  ...
  class Tx < CoinPrim
    ...
    def inspect
      "#<XP::Tx txid=#{@attrs['txid']} hash=#{@attrs['hash']}>"
    end
  end
  ...
  class TxOut < CoinPrim
    ...
    def inspect
      "#<XP::TxOut txid=#{@attrs['txid']} n=#{@attrs['n']}>"
    end
  end
  ...
  class TxIn < CoinPrim
    ...
    def inspect
      "#<XP::TxIn txid=#{@attrs['txid']} n=#{@attrs['n']} rtxid=#{@attrs['rtxid']} rn=#{@attrs['rn']}>"
    end
  end
  ...
  class Address < CoinPrim
    ...
    def inspect
      "#<XP::Address #{@attrs['address']} type=#{@attrs['type']}>"
    end
  end
  ...
  class UnspentTxOut < TxOut
    ...
    def inspect
      "#<XP::UnspentTxOut txid=#{@attrs['txid']} n=#{@attrs['n']} spendable=#{self.is_spendable?}>"
    end
  end
end
